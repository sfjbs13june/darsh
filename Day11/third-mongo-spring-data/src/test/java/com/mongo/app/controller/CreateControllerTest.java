...
public class CreateControllerTest {
    ...
    public void findByIdTest(){
        Customer expectedCustomer = new Customer("customer1", "1", "test address");
        when(customerRepository.findBycustId(anyString())).thenReturn(expectedCustomer);
        Customer res = createController.findByID("customer1");
        Assert.assertEquals(res.getName(),expectedCustomer.getName());
        Assert.assertEquals(res.getCustId(),expectedCustomer.getCustId());
